...
def main():
    ...
    for i in range(10):
        ...
        for j in range(exp_time):
            ...
            accMTCSC = assist.calc_acc(result_series_MTCSC)

            total_rms[i][0] += rms_MTCSC
            total_cost[i][0] += cost_MTCSC
            total_acc[i][0] += accMTCSC
            total_time[i][0] += time_cost

            print(f"Total_dirty RMS: {total_dirty_rms}")
            print(f"MTCSC RMS: {rms_MTCSC}, Cost: {cost_MTCSC}, Acc: {accMTCSC}, Time: {time_cost}")
            ...
            accscreen = assist.calc_acc(result_series_screen)

            total_rms[i][1] += rms_screen
            total_cost[i][1] += cost_screen
            total_acc[i][1] += accscreen
            total_time[i][1] += time_cost

            print(f"SCREEN RMS: {rms_screen}, Cost: {cost_screen}, Acc: {accscreen}, Time: {time_cost}")
            ...
            accspeedacc = assist.calc_acc(result_series_speedacc)

            total_rms[i][2] += rms_speedacc
            total_cost[i][2] += cost_speedacc
            total_acc[i][2] += accspeedacc
            total_time[i][2] += time_cost

            print(f"SpeedAcc RMS: {rms_speedacc}, Cost: {cost_speedacc}, Acc: {accspeedacc}, Time: {time_cost}")
            ...
            accexpsmooth = assist.calc_acc(result_series_expsmooth)

            total_rms[i][3] += rms_expsmooth
            total_cost[i][3] += cost_expsmooth
            total_acc[i][3] += accexpsmooth
            total_time[i][3] += time_cost

            print(f"EWMA RMS: {rms_expsmooth}, Cost: {cost_expsmooth}, Acc: {accexpsmooth}, Time: {time_cost}")
            ...
            acclsgreedy = assist.calc_acc(result_series_lsgreedy)

            total_rms[i][4] += rms_lsgreedy
            total_cost[i][4] += cost_lsgreedy
            total_acc[i][4] += acclsgreedy
            total_time[i][4] += time_cost

            print(f"LsGreedy RMS: {rms_lsgreedy}, Cost: {cost_lsgreedy}, Acc: {acclsgreedy}, Time: {time_cost}")

            # SHoTClean B
            dirty_series = assist.read_data(input_file_name, ",")
            dirty_series = assist.add_noise(dirty_series, drate, seed)
            SHoTClean_B_Multi_Soft = SHoTClean_B(dirty_series, s_max, s_min, is_soft=True)
            start_time = time.time()
            result_series_SHoTClean_B_Soft = SHoTClean_B_Multi_Soft.clean()
            end_time = time.time()
            ...
            print(
                f"SHoTClean B RMS: {rms_SHoTClean_B_Soft}, Cost: {cost_SHoTClean_B_Soft}, Acc: {acc_SHoTClean_B_Soft}, Time: {end_time - start_time}")
            ...
            accSHoTClean_S_Soft = assist.calc_acc(result_series_SHoTClean_S_M_Soft)

            total_rms[i][6] += rms_SHoTClean_S_Soft
            total_cost[i][6] += cost_SHoTClean_S_Soft
            total_acc[i][6] += accSHoTClean_S_Soft
            total_time[i][6] += end_time - start_time

            print(f"SHoTClean S RMS: {rms_SHoTClean_S_Soft}, Cost: {cost_SHoTClean_S_Soft}, Acc: {accSHoTClean_S_Soft}, Time: {end_time - start_time}")
            ...
            accSHoTClean_P_Soft = assist.calc_acc(result_series_SHoTClean_P_Soft)
            total_rms[i][7] += rms_SHoTClean_P_Soft
            total_cost[i][7] += cost_SHoTClean_P_Soft
            total_acc[i][7] += accSHoTClean_P_Soft
            total_time[i][7] += time_cost

            print(
                f"SHoTClean P RMS: {rms_SHoTClean_P_Soft}, Cost: {cost_SHoTClean_P_Soft}, Acc: {accSHoTClean_P_Soft}, Time: {time_cost}")
            ...
            accSHoTClean_C_Soft = assist.calc_acc(result_series_SHoTClean_C_Soft)

            total_rms[i][8] += rms_SHoTClean_C_Soft
            total_cost[i][8] += cost_SHoTClean_C_Soft
            total_acc[i][8] += accSHoTClean_C_Soft
            total_time[i][8] += end_time - start_time

            print(
                f"SHoTClean C RMS: {rms_SHoTClean_C_Soft}, Cost: {cost_SHoTClean_C_Soft}, Acc: {accSHoTClean_C_Soft}, Time: {end_time - start_time}")

            # Save Data
            # dirty_series = assist.read_data(input_file_name, ",")
            # dirty_series = assist.add_noise(dirty_series, drate, seed)
            # assist.save_data(dirty_series, f"../results/Multi/UCI/Dirty_{drate}.data")
            # assist.save_data(result_series_MTCSC, f"../results/Multi/UCI/MTCSC_{drate}.data")
            # assist.save_data(result_series_screen, f"../results/Multi/UCI/SCREEN_{drate}.data")
            # assist.save_data(result_series_speedacc, f"../results/Multi/UCI/SpeedAcc_{drate}.data")
            # assist.save_data(result_series_expsmooth, f"../results/Multi/UCI/EWMA_{drate}.data")
            # assist.save_data(result_series_lsgreedy, f"../results/Multi/UCI/LsGreedy_{drate}.data")
            # assist.save_data(result_series_SHoTClean_B_Soft, f"../results/Multi/UCI/B-Soft_{drate}.data")
            # assist.save_data(result_series_SHoTClean_C_Soft, f"../results/Multi/UCI/C-Soft_{drate}.data")
            # exit()
    ...
    print(total_acc)
    print(total_time)
    name = ["Methods" ,"MTCSC", "SCREEN", "SpeedAcc", "EWMA", "LsGreedy", "SHoTClean-B", "SHoTClean-S", "SHoTClean-P", "C-Soft"]
    write_file_name = "../results/Multi/UCI/RMS.csv"
    assist.write_csv(write_file_name, name, total_drate, total_rms)
    write_file_name = "../results/Multi/UCI/COST.csv"
    assist.write_csv(write_file_name, name, total_drate, total_cost)
    write_file_name = "../results/Multi/UCI/ACC.csv"
    assist.write_csv(write_file_name, name, total_drate, total_acc)
    write_file_name = "../results/Multi/UCI/TIME.csv"
