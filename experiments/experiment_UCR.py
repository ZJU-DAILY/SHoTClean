...
def main():
    ...
    input_file_name = f"UCR/db6.data"
    s_max = 0.500001
    s_min = -0.500001
    TT = 10
    ...
    for i in range(1):
        ...
        for j in range(exp_time):
            ...
            # BASE B
            dirty_series = assist.read_data(input_file_name, ",")
            BASE_B_Soft = BASE_B(dirty_series, s_max, s_min, is_soft=True)
            start_time = time.time()
            ...
            print(f"BASE C RMS: {rms_BASE_C_Soft}, Cost: {cost_BASE_C_Soft}, Acc: {acc_BASE_C_Soft}, Time: {end_time - start_time}")
